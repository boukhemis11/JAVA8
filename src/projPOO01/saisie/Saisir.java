...
public class Saisir {
	...
	public static void SaisirInterimaire(){
		...
		int duree = 0;
		boolean erreurcp = true;
		boolean erreurca = true;
		boolean erreurns =true;
		boolean erreurde =true;
		
		for(int i=0;i<5; i++) {
			System.out.println("Saisir le nom d'interimaire");
			nom = Menus.sc.next();
			System.out.println("Saisir le prenom d'interimaire");
			prenom = Menus.sc.next();
			System.out.println("Saisir l'adresse d'interimaire");
			adresse = Menus.sc.next();
			System.out.println("Saisir la ville d'interimaire");
			ville = Menus.sc.next();
			...
			while(erreurca) {
				try {
					System.out.println("Saisir le contrat d'interimaire");
					contratAgence = Menus.sc.next();
					Interimaire.CtrlContratAgence(contratAgence);
					erreurca = false;
			...
			while(erreurde) {
				try {
					System.out.println("Saisir la durée d'interimaire");
					duree = Menus.sc.nextInt();
					Interimaire.CtrlDuree(duree);
					erreurde = false;
				} catch (ExceptionDuree e) {
			...
			while(erreurns) {
				try {
					System.out.println("Saisir le numero de sécurité sociale d'interimaire");
					secu = Menus.sc.next();
					Salarie.CtrlSaisiNumeroSecu(secu);
					erreurns = false;
				} catch (ExceptionSaisiNumeroSecu e) {
			...
			salaire=0;
			System.out.println("Saisir le salaire d'interimaire");
			while(Menus.sc.hasNext()&& salaire == 0)
			{
